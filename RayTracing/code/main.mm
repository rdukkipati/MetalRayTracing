...
internal void BuildMetalLibraryPath(char *MetalLibraryPath) {
...
i32 main(i32 argc, const char *argv[]) {
  @autoreleasepool {
    ...
    NSString *NSMetalLibraryPath =
        [NSString stringWithUTF8String:MetalLibraryPath];

    id<MTLDevice> Device = MTLCreateSystemDefaultDevice();

    NSError *Errors;
    id<MTLLibrary> MetalLibrary = [Device newLibraryWithFile:NSMetalLibraryPath
                                                       error:&Errors];
    id<MTLFunction> VertexFunction =
        [Shaders newFunctionWithName:@"VertexFunction"];
    id<MTLFunction> FragmentFunction =
        [Shaders newFunctionWithName:@"FragmentFunction"];

    id<MTLCommandQueue> CommandQueue =
        [Device newCommandQueueWithMaxCommandBufferCount:64];
    ...
    id<MTLBuffer> VertexBuffer =
        [Device newBufferWithBytes:VerticesAndUVs
